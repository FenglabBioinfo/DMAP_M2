...
sub main{
	my %opts = &Help();
	my ($vcf,$fai,$win,$step,$out)=($opts{f},$opts{i},$opts{w},$opts{s},$opts{o});
	my @vcf_filtered_step1 = $opts{"c"} ? (&filter_step1($vcf,$opts{"c"})) : (&unfilter_vcf($vcf));
	my (%hash_fai,%hash_fai_2,@array_chr_name);
	&get_fai($fai,\%hash_fai,\%hash_fai_2,\@array_chr_name);
	my %hash_index=&filter_step2(\@vcf_filtered_step1,\%hash_fai,$out);
	my %hash_win=&get_win(\%hash_fai,$win,$step);
	my %hash_index_win=&slid_win(\%hash_index,\%hash_win,$win,$step);
	&create_Rscript(\%hash_index_win,\%hash_index,\@array_chr_name,\%hash_fai,\%hash_fai_2,$out,$fai);
	&plot($out);
}

sub filter_step1{
	my ($vcf,$common_variation)=@_;
	my (@vcf_filtered_step1,%hash_common_variation);
	if($common_variation =~ /.gz$/){
		open F1,"gzip -dc $common_variation|" or die $!;
	}else{
		open F1,$common_variation or die $!;
	}
	while(<F1>){
		chomp;
		next if(/^#/);
	...
	close F1;
	if($vcf =~ /.gz$/) {open F2,"gzip -dc $vcf|" or die $!}else{open F2,$vcf or die $!};
	while(<F2>){
		chomp;
		if(/^#/){
			push @vcf_filtered_step1,$_;
			next;
		}
		my ($chr,$loc,@tmp) = split;
		push @vcf_filtered_step1,$_ unless(exists $hash_common_variation{$chr}{$loc});
	}
	close F2;
...
sub unfilter_vcf{
	my $vcf=$_[0];
	my @vcf_filtered_step1;
	if($vcf =~ /.gz$/) {open F1,"gzip -dc $vcf|" or die $!}else{open F1,$vcf or die $!};
	while(<F1>){
		chomp;
		push @vcf_filtered_step1,$_;
	}
...
sub plot{
	...
	unlink (@file,"$out/index_all.tmp","$out/index_win.tmp","$out/SNP_filter_2.tsv");
}
...
sub Help{
	...
	my $usage = <<"USAGE";
Usage:
	perl $0  -f <vcf> -i <genome.fai> [options]
	--vcf	 -f	VCF files
	--index  -i	Genome index generated by samtools
	--win	 -w	Slide window, default 1000000
	--step	 -s	Step length, default 500000
	--common -c The VCF type file including the common variation
	--help	 -h	Help
Example:
	perl $0 --vcf ./Example/SNP.vcf --index ./Example/gmx.fa.fai --win 1000000 --step 500000 --out ./Example/out
	perl 3.DMAP_M2.pl --vcf ./Example/SNP.vcf --index ./Example/gmx.fa.fai --win 1000000 --step 500000 --out ./Example/out --common ./Example/common_variation.vcf.gz
Version: 
	Version 1.0, Date: 2020-12-02
Author:
...
sub create_Rscript{
	...
	close OUT_INDEX_WIN; close OUT_INDEX_ALL;
	my $R1=<<EOF;
...
fai<-read.table("$fai",sep = "\t",row.names = 1,header = F)
...
geom_line(data = index_win_chr,aes(Position,Dindex),colour="#F8766D",size=0.5)+
...
geom_line(data = index_win_chr,aes(Position,ABSDIndex),colour="#F8766D",size=0.5)+
...
geom_line(data = index_win_chr,aes(Position,MIndex),colour="#F8766D",size=0.5)+
scale_y_continuous(limits = c(0,1),breaks=seq(-1,1,0.25),expand=c(0,0))+
scale_x_continuous(limits = c(0,fai["$chr",1]),breaks = 10000000*0:10,labels = 10*0:10,expand=c(0,0))+
...
geom_line(data = index_win_chr,aes(Position,WIndex),colour="#F8766D",size=0.5)+
scale_y_continuous(limits = c(0,1),breaks=seq(-1,1,0.25),expand=c(0,0))+
scale_x_continuous(limits = c(0,fai["$chr",1]),breaks = 10000000*0:10,labels = 10*0:10,expand=c(0,0))+
...
sub filter_step2{
	my ($vcf_filtered,$hash_fai,$out)=@_;
	my %hash_index;
	...
	foreach my $line (@{$vcf_filtered}){
		if($line =~ /^#/){
			...
			my ($index_M,$index_W)=($depth_M_mut/$depth_M_all,$depth_W_mut/$depth_W_all);
			...
			my ($delta_index,$delta_index_abs)=(($index_M-$index_W),abs($index_M-$index_W));
			say F2 $line;
